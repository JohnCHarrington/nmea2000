...
def is_fast_pgn_126987() -> bool:
    ...
    return True
def decode_pgn_126987(_data_raw_: int, _data_length_bits_: int) -> NMEA2000Message:
    """Decode PGN 126987."""
    ...
    parameter_number = parameter_number_raw = decode_number(_data_raw_, running_bit_offset, 8, False, 1, 0, 252)
    ...
    trigger_method = trigger_method_raw = decode_number(_data_raw_, running_bit_offset, 8, False, 1, 0, 252)
    ...
    threshold_data_format = threshold_data_format_raw = decode_number(_data_raw_, running_bit_offset, 8, False, 1, 0, 252)
    ...
    threshold_level = threshold_level_raw = decode_number(_data_raw_, running_bit_offset, 64, False, 1, 0, 18446744073709551615)
    ...
    while (
        ...
        parameter_number = parameter_number_raw = decode_number(_data_raw_, running_bit_offset, 8, False, 1, 0, 252)
        running_bit_offset += 8
        repeating_entry["parameterNumber"] = _repeating_entry_value(parameter_number, parameter_number_raw)
    
        trigger_method = trigger_method_raw = decode_number(_data_raw_, running_bit_offset, 8, False, 1, 0, 252)
        running_bit_offset += 8
        repeating_entry["triggerMethod"] = _repeating_entry_value(trigger_method, trigger_method_raw)
    
        threshold_data_format = threshold_data_format_raw = decode_number(_data_raw_, running_bit_offset, 8, False, 1, 0, 252)
        running_bit_offset += 8
        repeating_entry["thresholdDataFormat"] = _repeating_entry_value(threshold_data_format, threshold_data_format_raw)
    
        threshold_level = threshold_level_raw = decode_number(_data_raw_, running_bit_offset, 64, False, 1, 0, 18446744073709551615)
        running_bit_offset += 64
        repeating_entry["thresholdLevel"] = _repeating_entry_value(threshold_level, threshold_level_raw)
...
def encode_pgn_126987(nmea2000Message: NMEA2000Message) -> bytes:
    ...
    for repeating_entry in repeating_field_set_1_entries:
        ...
        field = repeating_entry.get("parameterNumber")
        if field is None:
            raise ValueError("Cant encode this message, missing 'Parameter Number'")
        field_offset = running_bit_offset
        ...
        assert isinstance(field_value, int)
        if field_value < 0:
            raise ValueError("Cant encode this message, 'Parameter Number' cannot be negative")
        if field_bit_length < 0:
            raise ValueError("Cant encode this message, 'Parameter Number' has a negative bit length")
        if field_bit_length > 0 and field_value.bit_length() > field_bit_length:
            raise ValueError("Cant encode this message, 'Parameter Number' exceeds the encoded bit length")
        field_mask = (1 << field_bit_length) - 1 if field_bit_length > 0 else 0
        data_raw |= (field_value & field_mask) << field_offset
        payload_end_offset = field_offset + field_bit_length
        running_bit_offset = payload_end_offset if advance_running_offset else field_offset
        ...
        field = repeating_entry.get("triggerMethod")
        if field is None:
            raise ValueError("Cant encode this message, missing 'Trigger Method'")
        field_offset = running_bit_offset
        ...
        assert isinstance(field_value, int)
        if field_value < 0:
            raise ValueError("Cant encode this message, 'Trigger Method' cannot be negative")
        if field_bit_length < 0:
            raise ValueError("Cant encode this message, 'Trigger Method' has a negative bit length")
        if field_bit_length > 0 and field_value.bit_length() > field_bit_length:
            raise ValueError("Cant encode this message, 'Trigger Method' exceeds the encoded bit length")
        field_mask = (1 << field_bit_length) - 1 if field_bit_length > 0 else 0
        data_raw |= (field_value & field_mask) << field_offset
        payload_end_offset = field_offset + field_bit_length
        running_bit_offset = payload_end_offset if advance_running_offset else field_offset
        ...
        field = repeating_entry.get("thresholdDataFormat")
        if field is None:
            raise ValueError("Cant encode this message, missing 'Threshold Data Format'")
        field_offset = running_bit_offset
        ...
        assert isinstance(field_value, int)
        if field_value < 0:
            raise ValueError("Cant encode this message, 'Threshold Data Format' cannot be negative")
        if field_bit_length < 0:
            raise ValueError("Cant encode this message, 'Threshold Data Format' has a negative bit length")
        if field_bit_length > 0 and field_value.bit_length() > field_bit_length:
            raise ValueError("Cant encode this message, 'Threshold Data Format' exceeds the encoded bit length")
        field_mask = (1 << field_bit_length) - 1 if field_bit_length > 0 else 0
        data_raw |= (field_value & field_mask) << field_offset
        payload_end_offset = field_offset + field_bit_length
        running_bit_offset = payload_end_offset if advance_running_offset else field_offset
        ...
        field = repeating_entry.get("thresholdLevel")
        if field is None:
            raise ValueError("Cant encode this message, missing 'Threshold Level'")
        field_offset = running_bit_offset
        ...
        assert isinstance(field_value, int)
        if field_value < 0:
            raise ValueError("Cant encode this message, 'Threshold Level' cannot be negative")
        if field_bit_length < 0:
            raise ValueError("Cant encode this message, 'Threshold Level' has a negative bit length")
        if field_bit_length > 0 and field_value.bit_length() > field_bit_length:
            raise ValueError("Cant encode this message, 'Threshold Level' exceeds the encoded bit length")
        field_mask = (1 << field_bit_length) - 1 if field_bit_length > 0 else 0
        data_raw |= (field_value & field_mask) << field_offset
        payload_end_offset = field_offset + field_bit_length
        running_bit_offset = payload_end_offset if advance_running_offset else field_offset
