...
def is_fast_pgn_127504() -> bool:
    ...
    return True
def decode_pgn_127504(_data_raw_: int, _data_length_bits_: int) -> NMEA2000Message:
    """Decode PGN 127504."""
    ...
    while (
        ...
        waveform = master_dict['WAVEFORM'].get(waveform_raw, None)
        running_bit_offset += 3
        repeating_entry["waveform"] = _repeating_entry_value(waveform, waveform_raw)
    
        reserved_21 = reserved_21_raw = decode_int(_data_raw_, running_bit_offset, 3)
        running_bit_offset += 3
        repeating_entry["reserved_21"] = _repeating_entry_value(reserved_21, reserved_21_raw)
...
def encode_pgn_127504(nmea2000Message: NMEA2000Message) -> bytes:
    ...
    for repeating_entry in repeating_field_set_1_entries:
        ...
        if isinstance(field.raw_value, int):
            ...
            field_value = lookup_encode_LINE(field.value)
        field_bit_length = 2
        assert isinstance(field_value, int)
        if field_value < 0:
        ...
        field = repeating_entry.get("waveform")
        if field is None:
            raise ValueError("Cant encode this message, missing 'Waveform'")
        field_offset = running_bit_offset
        ...
        if isinstance(field.raw_value, int):
            ...
            field_value = lookup_encode_WAVEFORM(field.value)
        field_bit_length = 3
        assert isinstance(field_value, int)
        if field_value < 0:
            raise ValueError("Cant encode this message, 'Waveform' cannot be negative")
        if field_bit_length < 0:
            raise ValueError("Cant encode this message, 'Waveform' has a negative bit length")
        if field_bit_length > 0 and field_value.bit_length() > field_bit_length:
            raise ValueError("Cant encode this message, 'Waveform' exceeds the encoded bit length")
        field_mask = (1 << field_bit_length) - 1 if field_bit_length > 0 else 0
        data_raw |= (field_value & field_mask) << field_offset
        payload_end_offset = field_offset + field_bit_length
        running_bit_offset = payload_end_offset if advance_running_offset else field_offset
        ...
        field = repeating_entry.get("reserved_21")
        if field is None:
