...
def decode_pgn_130821(data_raw: int, data_length_bits: int | None = None) -> NMEA2000Message | None:
    ...
    if (
        ...
        return decode_pgn_130821_navicoAsciiData(data_raw, data_length_bits)
    ...
    if (
        ...
        return decode_pgn_130821_furunoUnknown130821(data_raw, data_length_bits)
    ...
    if (
        ...
        return decode_pgn_130821_maretronAlertText(data_raw, data_length_bits)
    
    
    return None
    
def decode_pgn_130821_navicoAsciiData(_data_raw_: int, _data_length_bits_: int) -> NMEA2000Message:
    """Decode PGN 130821."""
    ...
    message, message_raw = decode_string_fix_raw(_data_raw_, running_bit_offset, 1840)
...
def encode_pgn_130821_navicoAsciiData(nmea2000Message: NMEA2000Message) -> bytes:
    ...
    advance_running_offset = True
    field_value = encode_string_fix(field.raw_value if isinstance(field.raw_value, (bytes, bytearray, memoryview)) else (field.value if field.value is not None else field.raw_value), 1840)
    field_bit_length = 1840
    assert isinstance(field_value, int)
    if field_value < 0:
        raise ValueError("Cant encode this message, 'Message' cannot be negative")
    if field_bit_length < 0:
        raise ValueError("Cant encode this message, 'Message' has a negative bit length")
    if field_bit_length > 0 and field_value.bit_length() > field_bit_length:
        raise ValueError("Cant encode this message, 'Message' exceeds the encoded bit length")
    field_mask = (1 << field_bit_length) - 1 if field_bit_length > 0 else 0
    data_raw |= (field_value & field_mask) << field_offset
    payload_end_offset = field_offset + field_bit_length
    running_bit_offset = payload_end_offset if advance_running_offset else field_offset
...
def decode_pgn_130821_furunoUnknown130821(_data_raw_: int, _data_length_bits_: int) -> NMEA2000Message:
    """Decode PGN 130821."""
    ...
    h = h_raw = decode_number(_data_raw_, running_bit_offset, 8, False, 1, 0, 252)
    ...
    i = i_raw = decode_number(_data_raw_, running_bit_offset, 8, False, 1, 0, 252)
...
def decode_pgn_130821_maretronAlertText(_data_raw_: int, _data_length_bits_: int) -> NMEA2000Message:
    """Decode PGN 130821."""
