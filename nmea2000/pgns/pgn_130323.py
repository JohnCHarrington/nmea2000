...
def is_fast_pgn_130323() -> bool:
    ...
    return True
def decode_pgn_130323(_data_raw_: int, _data_length_bits_: int) -> NMEA2000Message:
    """Decode PGN 130323."""
    ...
    ambient_temperature = ambient_temperature_raw = decode_number(_data_raw_, running_bit_offset, 16, False, 0.01, 0, 655.32)
...
def encode_pgn_130323(nmea2000Message: NMEA2000Message) -> bytes:
    ...
    assert isinstance(field_value, int)
    if field_value < 0:
        raise ValueError("Cant encode this message, 'Ambient Temperature' cannot be negative")
    if field_bit_length < 0:
        raise ValueError("Cant encode this message, 'Ambient Temperature' has a negative bit length")
    if field_bit_length > 0 and field_value.bit_length() > field_bit_length:
        raise ValueError("Cant encode this message, 'Ambient Temperature' exceeds the encoded bit length")
    field_mask = (1 << field_bit_length) - 1 if field_bit_length > 0 else 0
    data_raw |= (field_value & field_mask) << field_offset
    payload_end_offset = field_offset + field_bit_length
    running_bit_offset = payload_end_offset if advance_running_offset else field_offset
    payload_bit_length = max(payload_bit_length, payload_end_offset)
    # stationId | Offset: 208, Length: , Resolution: , Field Type: STRING_LAU
    running_bit_offset = 208
    field_offset = running_bit_offset
