...
def decode_pgn_65285(data_raw: int, data_length_bits: int | None = None) -> NMEA2000Message | None:
    ...
    if (
        ...
        return decode_pgn_65285_airmarBootStateAcknowledgment(data_raw, data_length_bits)
    ...
    if (
        ...
        return decode_pgn_65285_lowranceTemperature(data_raw, data_length_bits)
    ...
    if (
        ...
        return decode_pgn_65285_maretronUniversalConfigurationSf(data_raw, data_length_bits)
    
    
    return None
    
def decode_pgn_65285_airmarBootStateAcknowledgment(_data_raw_: int, _data_length_bits_: int) -> NMEA2000Message:
    """Decode PGN 65285."""
...
def encode_pgn_65285_airmarBootStateAcknowledgment(nmea2000Message: NMEA2000Message) -> bytes:
    ...
    if isinstance(field.raw_value, int):
        ...
        field_value = lookup_encode_BOOT_STATE(field.value)
    field_bit_length = 3
    assert isinstance(field_value, int)
    if field_value < 0:
        raise ValueError("Cant encode this message, 'Boot State' cannot be negative")
    if field_bit_length < 0:
        raise ValueError("Cant encode this message, 'Boot State' has a negative bit length")
    if field_bit_length > 0 and field_value.bit_length() > field_bit_length:
        raise ValueError("Cant encode this message, 'Boot State' exceeds the encoded bit length")
    field_mask = (1 << field_bit_length) - 1 if field_bit_length > 0 else 0
    data_raw |= (field_value & field_mask) << field_offset
    payload_end_offset = field_offset + field_bit_length
    running_bit_offset = payload_end_offset if advance_running_offset else field_offset
...
def decode_pgn_65285_lowranceTemperature(_data_raw_: int, _data_length_bits_: int) -> NMEA2000Message:
    """Decode PGN 65285."""
    ...
    actual_temperature = actual_temperature_raw = decode_number(_data_raw_, running_bit_offset, 16, False, 0.01, 0, 655.32)
...
def decode_pgn_65285_maretronUniversalConfigurationSf(_data_raw_: int, _data_length_bits_: int) -> NMEA2000Message:
    """Decode PGN 65285."""
