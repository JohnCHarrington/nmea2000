...
def is_fast_pgn_65360() -> bool:
    ...
    return False
def decode_pgn_65360(_data_raw_: int, _data_length_bits_: int) -> NMEA2000Message:
    """Decode PGN 65360."""
    ...
    target_heading_true = target_heading_true_raw = decode_number(_data_raw_, running_bit_offset, 16, False, 0.0001, 0, 6.2831852)
    ...
    target_heading_magnetic = target_heading_magnetic_raw = decode_number(_data_raw_, running_bit_offset, 16, False, 0.0001, 0, 6.2831852)
...
def encode_pgn_65360(nmea2000Message: NMEA2000Message) -> bytes:
    ...
    assert isinstance(field_value, int)
    if field_value < 0:
        raise ValueError("Cant encode this message, 'Target Heading True' cannot be negative")
    if field_bit_length < 0:
        raise ValueError("Cant encode this message, 'Target Heading True' has a negative bit length")
    if field_bit_length > 0 and field_value.bit_length() > field_bit_length:
        raise ValueError("Cant encode this message, 'Target Heading True' exceeds the encoded bit length")
    field_mask = (1 << field_bit_length) - 1 if field_bit_length > 0 else 0
    data_raw |= (field_value & field_mask) << field_offset
    payload_end_offset = field_offset + field_bit_length
    running_bit_offset = payload_end_offset if advance_running_offset else field_offset
    ...
    assert isinstance(field_value, int)
    if field_value < 0:
        raise ValueError("Cant encode this message, 'Target Heading Magnetic' cannot be negative")
    if field_bit_length < 0:
        raise ValueError("Cant encode this message, 'Target Heading Magnetic' has a negative bit length")
    if field_bit_length > 0 and field_value.bit_length() > field_bit_length:
        raise ValueError("Cant encode this message, 'Target Heading Magnetic' exceeds the encoded bit length")
    field_mask = (1 << field_bit_length) - 1 if field_bit_length > 0 else 0
    data_raw |= (field_value & field_mask) << field_offset
    payload_end_offset = field_offset + field_bit_length
    running_bit_offset = payload_end_offset if advance_running_offset else field_offset
