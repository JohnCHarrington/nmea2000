...
def is_fast_pgn_65408() -> bool:
    ...
    return False
def decode_pgn_65408(_data_raw_: int, _data_length_bits_: int) -> NMEA2000Message:
    """Decode PGN 65408."""
...
def encode_pgn_65408(nmea2000Message: NMEA2000Message) -> bytes:
    ...
    if isinstance(field.raw_value, int):
        ...
        field_value = lookup_encode_AIRMAR_DEPTH_QUALITY_FACTOR(field.value)
    field_bit_length = 4
    assert isinstance(field_value, int)
    if field_value < 0:
        raise ValueError("Cant encode this message, 'Depth Quality Factor' cannot be negative")
    if field_bit_length < 0:
        raise ValueError("Cant encode this message, 'Depth Quality Factor' has a negative bit length")
    if field_bit_length > 0 and field_value.bit_length() > field_bit_length:
        raise ValueError("Cant encode this message, 'Depth Quality Factor' exceeds the encoded bit length")
    field_mask = (1 << field_bit_length) - 1 if field_bit_length > 0 else 0
    data_raw |= (field_value & field_mask) << field_offset
    payload_end_offset = field_offset + field_bit_length
    running_bit_offset = payload_end_offset if advance_running_offset else field_offset
