...
def is_fast_pgn_126984() -> bool:
    ...
    return True
def decode_pgn_126984(_data_raw_: int, _data_length_bits_: int) -> NMEA2000Message:
    """Decode PGN 126984."""
...
def encode_pgn_126984(nmea2000Message: NMEA2000Message) -> bytes:
    ...
    if isinstance(field.raw_value, int):
        ...
        field_value = lookup_encode_ALERT_RESPONSE_COMMAND(field.value)
    field_bit_length = 2
    assert isinstance(field_value, int)
    if field_value < 0:
        raise ValueError("Cant encode this message, 'Response Command' cannot be negative")
    if field_bit_length < 0:
        raise ValueError("Cant encode this message, 'Response Command' has a negative bit length")
    if field_bit_length > 0 and field_value.bit_length() > field_bit_length:
        raise ValueError("Cant encode this message, 'Response Command' exceeds the encoded bit length")
    field_mask = (1 << field_bit_length) - 1 if field_bit_length > 0 else 0
    data_raw |= (field_value & field_mask) << field_offset
    payload_end_offset = field_offset + field_bit_length
    running_bit_offset = payload_end_offset if advance_running_offset else field_offset
